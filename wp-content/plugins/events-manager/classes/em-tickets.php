...
class EM_Tickets extends EM_Object implements Iterator, Countable {
	...
	function __construct( $object = false ){
		...
		if( is_numeric($object) || (is_object($object) && in_array(get_class($object), array("EM_Event","EM_Booking"))) ){
			...
			$ticket_orderby_options = apply_filters('em_tickets_orderby_options', array(
				...
				'ticket_name DESC, ticket_price DES C'=>__('Ticket Name (Descending)','events-manager')
			));
	...
	function delete(){
		...
		if( !empty($this->tickets) ){
			...
			if(count($ticket_ids) > 0){
				...
				if( $bookings > 0 ){
					$result = false;
					$this->add_error(pll__('You cannot delete tickets if there are any bookings associated with them. Please delete these bookings first.','events-manager'));
				}else{
	...
	function get_post(){
		...
		if( !empty($_POST['em_tickets']) && is_array($_POST['em_tickets']) ){
			...
			$EM_Ticket = new EM_Ticket(array(
				...
				'ticket_name' => pll__('Standard','events-manager')
			));
			$this->tickets[] = $EM_Ticket;
		}
	...
	function get_ticket_collumns($EM_Event = false){
		...
		$collumns = array( 'type' => pll__('Date','events-manager'), 'price' => pll__('Price','events-manager'), 'spaces' => pll__('Spaces','events-manager'));
