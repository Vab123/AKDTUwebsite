...
class EM_Event extends EM_Object {
	...
	function __construct($id = false, $search_by = 'event_id') {
		...
		if (empty($this->event_timezone)) {
			if (get_option('dbem_timezone_enabled')) {
				...
				if ($this->event_timezone == 'UTC+0' || $this->event_timezone == 'UTC +0') {
					$this->event_timezone = 'UTC';
				}
			} else {
	...
	function validate_meta() {
		...
		if (preg_match('/\d{4}-\d{2}-\d{2}/', $this->event_start_date) && preg_match('/\d{4}-\d{2}-\d{2}/', $this->event_end_date)) {
			...
			if (!empty($missing_fields['event_start_date'])) {
				unset($missing_fields['event_start_date']);
			}
			if (!empty($missing_fields['event_end_date'])) {
				unset($missing_fields['event_end_date']);
			}
	...
	function get_edit_url() {
		if ($this->can_manage('edit_events', 'edit_others_events')) {
			if (EM_MS_GLOBAL && get_site_option('dbem_ms_global_events_links') && !empty($this->blog_id) && is_main_site() && $this->blog_id != get_current_blog_id()) {
				if (get_blog_option($this->blog_id, 'dbem_edit_events_page' . (pll_current_language() == "en" ? '_english' : ''))) {
					$link = em_add_get_params(get_permalink(get_blog_option($this->blog_id, 'dbem_edit_events_page' . (pll_current_language() == "en" ? '_english' : ''))), array('action' => 'edit', 'event_id' => $this->event_id), false);
				}
				if (empty($link))
					$link = get_admin_url($this->blog_id, "post.php?post={$this->post_id}&action=edit");
			} else {
				if (get_option('dbem_edit_events_page' . (pll_current_language() == "en" ? '_english' : '')) && !is_admin()) {
					$link = em_add_get_params(get_permalink(get_option('dbem_edit_events_page' . (pll_current_language() == "en" ? '_english' : ''))), array('action' => 'edit', 'event_id' => $this->event_id), false);
				}
				if (empty($link))
				...
				case '#_EVENTDATES_LOCAL':
					if (!defined('EM_JS_MOMENTJS_PH') || EM_JS_MOMENTJS_PH) {
						...
						if (empty($time_format)) {
							...
							$replacements = array(
								/* Day */
								'jS' => 'Do', /*o doesn't exist on its own, so we find/replase jS only*/ 'd' => 'DD', 'D' => 'ddd', 'j' => 'D', 'l' => 'dddd', 'N' => 'E', /*'S' => 'o' - see jS*/ 'w' => 'e', 'z' => 'DDD',
								/* Week */ 'W' => 'W',
...
<?php
				...
				case '#_AVAILABLESPACES':
					if ($this->event_rsvp && get_option('dbem_rsvp_enabled')) {
						$translations = pll_get_post_translations($this->post_id);
