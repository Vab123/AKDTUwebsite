...
class EM_Booking extends EM_Object {
	...
	function __construct($booking_data = false) {
		...
		$this->status_array = array(
			0 => pll__('Pending', 'events-manager'),
			1 => pll__('Approved', 'events-manager'),
			2 => pll__('Rejected', 'events-manager'),
			3 => pll__('Cancelled', 'events-manager'),
			4 => pll__('Awaiting Online Payment', 'events-manager'),
			5 => pll__('Awaiting Payment', 'events-manager')
		);
	...
	function validate($override_availability = false) {
		...
		if ($this->booking_spaces == 0) {
			$this->add_error(get_option('dbem_booking_feedback_min_space_' . pll_current_language()));
		}
		...
		if (!$override_availability) {
			...
			if ($this->get_event()->get_bookings()->get_available_spaces() < $this->get_spaces()) {
				$result = false;
				$this->add_error(get_option('dbem_booking_feedback_full_' . pll_current_language()));
			}
		}
		...
		if ($this->get_event()->event_rsvp_spaces > 0 && $this->get_spaces() > $this->get_event()->event_rsvp_spaces) {
			$result = false;
			$this->add_error(sprintf(get_option('dbem_booking_feedback_spaces_limit_' . pll_current_language()), $this->get_event()->event_rsvp_spaces));
		}
	...
	function get_person_post() {
		...
		if ($user_email == '') {
			...
			$registration = false;
			$this->add_error(get_option('dbem_booking_feedback_email_exists_' . pll_current_language()));
		} else {
...
?>
		<table class="em-form-fields">
			<tr>
				...
				<td><input type="text" name="user_name" value="<?php echo esc_attr($name); ?>" /></td>
			</tr>
			<tr>
				<th><?php _e('Email', 'events-manager'); ?> : </th>
				<td><input type="text" name="user_email" value="<?php echo esc_attr($email); ?>" /></td>
			</tr>
			<tr>
				<th><?php _e('Phone', 'events-manager'); ?> : </th>
				<td><input type="text" name="dbem_phone" value="<?php echo esc_attr($phone); ?>" /></td>
			</tr>
		</table>
<?php
	...
	function output($format, $target = "html") {
		...
		foreach ($this->get_tickets() as $EM_Ticket) { /* @var $EM_Ticket EM_Ticket */
			break;
	...
	function email($email_admin = true, $force_resend = false, $email_attendee = true) {
		...
		if ($this->booking_status !== $this->previous_status || $force_resend) {
			...
			if (!empty($msg['user']['subject']) && $email_attendee) {
				...
				$msg['user']['body'] = $this->output($msg['user']['body'], 'html');
				$attachments = array();
			...
			if ($email_admin && !empty($msg['admin']['subject'])) { //emails won't be sent if admin is logged in unless they book themselves
				...
				foreach ($admin_emails as $key => $email) {
					if (!is_email($email)) unset($admin_emails[$key]);
				} //remove bad emails
				...
				if (!empty($admin_emails)) {
					...
					$msg['admin']['body'] = $this->output($msg['admin']['body'], 'html');
					$attachments = array();
	...
	function email_messages() {
		...
		switch ($this->booking_status) {
			...
			case 5: //TODO remove offline status from here and move to pro
				...
				break;
			case 1:
				$msg['user']['subject'] = get_option('dbem_bookings_email_confirmed_subject_' . pll_current_language());
				$msg['user']['body'] = get_option('dbem_bookings_email_confirmed_body_' . pll_current_language());
				//admins should get something (if set to)
			...
			case 2:
				...
				break;
			case 3:
				$msg['user']['subject'] = get_option('dbem_bookings_email_cancelled_subject_' . pll_current_language());
				$msg['user']['body'] = get_option('dbem_bookings_email_cancelled_body_' . pll_current_language());
				//admins should get something (if set to)
