	...
	function add( $EM_Booking ){
		...
		if($result){
			...
			if( get_option('dbem_bookings_approval') == 1 && $EM_Booking->booking_status == 0){
				$this->feedback_message = get_option('dbem_booking_feedback_pending_' . pll_current_language());
			}else{
				$this->feedback_message = get_option('dbem_booking_feedback_' . pll_current_language());
			}
			if(!$email){
				$EM_Booking->email_not_sent = true;
				$this->feedback_message .= ' '.get_option('dbem_booking_feedback_nomail_' . pll_current_language());
			...
			//Failure
			$this->errors[] = "<strong>".get_option('dbem_booking_feedback_error_' . pll_current_language())."</strong><br />". implode('<br />', $EM_Booking->errors);
		}
