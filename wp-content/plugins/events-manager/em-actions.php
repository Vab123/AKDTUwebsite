...
function em_init_actions() {
	...
	if( !empty($_REQUEST['action']) && substr($_REQUEST['action'],0,5) == 'event' ){
		...
		if( $_REQUEST['action'] == 'event_save' && $EM_Event->can_manage('edit_events','edit_others_events') ){
			...
			if ( $EM_Event->get_post() && $EM_Event->save() ) { //EM_Event gets the event if submitted via POST and validates it (safer than to depend on JS)
				...
				if( is_user_logged_in() ){
					if( empty($_REQUEST['event_id']) ){
						$EM_Notices->add_confirm( $EM_Event->output(get_option('dbem_events_form_result_success_' . (count(array_filter(wp_get_current_user()->roles,function($role){return $role == 'vicevaert';})) > 0 ? 'vicevært_' : '') . pll_get_post_language($EM_Event->post_id,'slug'))), true);
					}else{
					    $EM_Notices->add_confirm( $EM_Event->output(get_option('dbem_events_form_result_success_updated_' . (count(array_filter(wp_get_current_user()->roles,function($role){return $role == 'vicevaert';})) > 0 ? 'vicevært_' : '') . pll_get_post_language($EM_Event->post_id,'slug'))), true);
					}
				}else{
		...
		if ( $_REQUEST['action'] == 'event_delete' && wp_verify_nonce($_REQUEST['_wpnonce'],'event_delete_'.$EM_Event->event_id) ) { 
			//DELETE action
			$selectedEvents = !empty($_REQUEST['event_id']) ? $_REQUEST['event_id']:'';
			...
			$plural = __('Event','events-manager');
			if($events_result){
	...
	if( !empty($_REQUEST['action']) && substr($_REQUEST['action'],0,7) == 'booking' && (is_user_logged_in() || ($_REQUEST['action'] == 'booking_add' && get_option('dbem_bookings_anonymous'))) ){
		...
		if ( $_REQUEST['action'] == 'booking_add') {
			...
			if( !is_user_logged_in() || get_option('dbem_bookings_double') || !$EM_Event->get_bookings()->has_booking(get_current_user_id()) ){
				...
				$result = false;
				$feedback = get_option('dbem_booking_feedback_already_booked_' . pll_current_language());
				...
				if( $EM_Booking->cancel() ){
					...
					if( !defined('DOING_AJAX') ){
						if( $EM_Booking->person->ID == get_current_user_id() ){
							$EM_Notices->add_confirm(get_option('dbem_booking_feedback_cancelled_' . pll_current_language()), true );
						}else{
		...
		$EM_Notices->add_error( get_option('dbem_booking_feedback_log_in_' . pll_current_language()) );
		if( !$result && defined('DOING_AJAX') ){
	...
	if( is_user_logged_in() ){
		...
		if( !empty($_REQUEST['em_obj']) && $_REQUEST['em_obj'] == 'em_bookings_events_print_table' ){
