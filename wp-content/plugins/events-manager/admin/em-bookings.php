...
add_action('em_bookings_add_user_ticket','add_user_signup');
	...
	<div class='wrap em-bookings-dashboard'>
		<?php if( is_admin() ): ?>
  		...
  		<div class="em-bookings-recent">
			...
			$EM_Bookings_Table->status = 'confirmed';
			...
			$EM_Bookings_Table->cols_template['ticket_name'] = __('Date', 'events-manager');
	...
	<div class='wrap'>
		...
		<div>
			...
			<p>
				...
				echo $booked_spaces."/".$EM_Event->get_spaces() ." ". __('Spaces confirmed','events-manager'); ?>
				<?php if( get_option('dbem_bookings_approval_reserved') ): ?>
				, <?php echo $EM_Event->get_spaces() - $booked_spaces . '/'. $EM_Event->get_spaces() ." ". __('Available spaces','events-manager'); ?>
				<?php endif; ?>
			</p>
			<p>
		...
		$EM_Bookings_Table->output();
  		?>
		<h2><?php pll_e('Add booking','events-manager'); ?></h2>
	...
	if (isset($_REQUEST['user']) && $_REQUEST['user'] > 0 && isset($_REQUEST['event_id']) && wp_verify_nonce($_REQUEST['_wpnonce'],'add_user_ticket_' . $_REQUEST['event_id'])) {
		if ($wpdb->get_var("SELECT COUNT(*) FROM " . EM_BOOKINGS_TABLE . " WHERE event_id=" . $_REQUEST['event_id'] . " AND person_id=" . $_REQUEST['user']) > 0) : ?>
			<div class="notice notice-warning is-dismissible">
				<p><?php pll_e('User was already signed up. Attempting to add additional signup', 'events-manager'); ?></p>
			</div>
		<?php endif;
