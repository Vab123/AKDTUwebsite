...
function em_admin_menu(){
	...
	$both_num = ($both_pending_count > 0) ? '<span class="update-plugins count-'.$both_pending_count.'"><span class="plugin-count">'.$both_pending_count.'</span></span>':'';
  	// Add a submenu to the custom top-level menu:
   	$plugin_pages = array();
	$plugin_pages['signuplists'] = add_submenu_page('edit.php?post_type='.EM_POST_TYPE_EVENT, pll__('Tilmeldingslister', 'events-manager'), pll__('Tilmeldingslister', 'events-manager'), 'manage_bookings', 'events-manager-print-bookings', "em_bookings_print_page");
   	if( get_option('dbem_rsvp_enabled') ){
	...
	if (!current_user_can('publish_events')){;
		foreach ($menu as $key => $value) {
			if ($value[0] == 'Events' || $value[0] == 'Arrangementer') {
				unset($menu[$key]);
				break;
			}
		}
