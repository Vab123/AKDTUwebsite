...
function em_content($page_content) {
	...
	$edit_events_page_id_english = get_option( 'dbem_edit_events_page_english' );
	...
	$my_bookings_page_id_english = get_option( 'dbem_my_bookings_page_english' );
	//general defaults
	$args = array(				
		'owner' => false,
		'pagination' => 1,
	...
	if( !post_password_required() && in_array($post->ID, array($events_page_id, $locations_page_id, $categories_page_id, $edit_bookings_page_id, $edit_events_page_id, $edit_events_page_id_english, $edit_locations_page_id, $my_bookings_page_id, $my_bookings_page_id_english, $tags_page_id)) ){
		get_post();
		...
		if( empty($content) ){
			...
			if ( $post->ID == $events_page_id && $events_page_id != 0 ) {
				...
				if( !empty($args['ajax']) ) echo "</div>"; //AJAX wrapper close
			}elseif( ($post->ID == $edit_events_page_id || $post->ID == $edit_events_page_id_english) && $edit_events_page_id != 0 && $edit_events_page_id_english != 0 ){
				em_events_admin();
			}elseif( $post->ID == $edit_locations_page_id && $edit_locations_page_id != 0 ){
				em_locations_admin();
			}elseif( ($post->ID == $my_bookings_page_id || $post->ID == $my_bookings_page_id_english) && $my_bookings_page_id != 0 && $my_bookings_page_id_english != 0 ){
				em_my_bookings();
...
function em_content_page_title($original_content, $id = null) {
	...
	$edit_locations_page_id = get_option( 'dbem_edit_locations_page' );
	$edit_bookings_page_id = get_option( 'dbem_edit_bookings_page' );
	if( !empty($post->ID) && in_array($post->ID, array($events_page_id, $locations_page_id, $edit_events_page_id, $edit_events_page_id_english, $edit_locations_page_id, $edit_bookings_page_id))){
		...
		if( empty($content) ){
			...
			}elseif( $post->ID == $edit_events_page_id || $post->ID == $edit_events_page_id_english ){
