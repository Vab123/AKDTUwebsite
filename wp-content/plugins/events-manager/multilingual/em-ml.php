...
class EM_ML{
	...
	public static function init(){
		...
		self::$wplang = apply_filters('em_ml_wplang', pll_current_language());
		self::$current_language = !empty($_REQUEST['em_lang']) && array_key_exists($_REQUEST['em_lang'], self::$langs) ? $_REQUEST['em_lang'] : pll_current_language();
	 ...
	 * preferred locale rather than the pll_current_language() which is not what we'd want in these situations. Therefore when switching language this hook is active.
	 * @param $locale
	 * @return mixed|string
	 */
