...
<div id="em-booking" class="em-booking <?php if( get_option('dbem_css_rsvp') ) echo 'css-booking'; ?>">
	...
	<?php if( $has_booking && !get_option('dbem_bookings_double') ): //Double bookings not allowed ?>
		<p>
			<?php pll_e( 'You are already signed up for this event.', 'events-manager' ); ?>
			<a href="<?php echo em_get_my_bookings_url(); ?>"><?php pll_e( 'Manage my signups', 'events-manager' ); ?></a>
		</p>
	<?php elseif( !$EM_Event->event_rsvp ): //bookings not enabled ?>
		<p><?php echo get_option('dbem_bookings_form_msg_disabled_' . pll_current_language()); ?></p>
	<?php elseif( $EM_Event->get_bookings()->get_available_spaces() <= 0 ): ?>
		<p><?php echo get_option('dbem_bookings_form_msg_full_' . pll_current_language()); ?></p>
	<?php elseif( !$is_open ): //event has started ?>
		<p><?php echo get_option('dbem_bookings_form_msg_closed_' . pll_current_language());  ?></p>
	<?php else: ?>
		<?php echo $EM_Notices; ?>
		<?php if( $tickets_count > 0) : ?>
			...
			<form class="em-booking-form" name='booking-form' method='post' action='<?php echo apply_filters('em_booking_form_action_url',''); ?>#em-booking'>
				...
				?>
				<?php if( $can_book ): ?>
					<div class='em-booking-form-details' style="width:100%">
						<?php 
						...
						<div class="em-booking-buttons">
							<?php if( preg_match('/https?:\/\//',get_option('dbem_bookings_submit_button_' . pll_current_language())) ): //Settings have an image url (we assume). Use it here as the button.?>
							<input type="image" src="<?php echo get_option('dbem_bookings_submit_button_' . pll_current_language()); ?>" class="em-booking-submit" id="em-booking-submit" />
							<?php else: //Display normal submit button ?>
							<input type="submit" class="<?php if(is_admin()) echo 'button-primary '; ?>em-booking-submit" id="em-booking-submit" value="<?php echo esc_attr(get_option('dbem_bookings_submit_button_' . pll_current_language())); ?>" />
							<?php endif; ?>
						</div>
						<?php do_action('em_booking_form_footer_after_buttons', $EM_Event); //do not delete ?>
					</div>
				<?php else: ?>
					<p class="em-booking-form-details"><?php echo get_option('dbem_booking_feedback_log_in_' . pll_current_language()); ?></p>
				<?php endif; ?>
			</form>	
			<?php 
