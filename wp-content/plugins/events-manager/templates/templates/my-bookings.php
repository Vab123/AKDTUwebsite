...
<?php
	...
	if( is_user_logged_in() ):
		...
		<div class='em-my-bookings'>
								...
								<tr>
									...
									<td>
										...
										if( !in_array($EM_Booking->booking_status, array(2,3)) && get_option('dbem_bookings_user_cancellation') && $EM_Event->get_bookings()->has_open_time() ){
											...
											$cancel_link = '<a class="em-bookings-cancel" href="'.$cancel_url.'" onclick="if( !confirm(EM.booking_warning_cancel) ){ return false; }">'.pll__('Cancel','events-manager').'</a>';
										}
				...
				<?php else: ?>
					<?php pll_e('You do not have any bookings.', 'events-manager'); ?>
				<?php endif; ?>
...
<?php else: ?>
	<p><?php echo sprintf(pll__('Please <a href="%s">Log In</a> to view your bookings.','events-manager'),site_url('wp-login.php?redirect_to=' . urlencode(get_permalink()), 'login'))?></p>
<?php endif; ?>