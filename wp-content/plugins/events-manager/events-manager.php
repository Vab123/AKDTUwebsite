<?php
/*
Plugin Name: AKDTU - Havedag tilmelding og fælleshus styring
Version: 99.9.9
Description: MÅ IKKE OPDATERES! - Bygget på Events Manager af Marcus Sykes
...
if( is_admin() ){
	...
	include('admin/em-print-bookings.php');
	...
	//bookings folder
		...
		include('admin/bookings/em-print-events.php');
...
class EM_Scripts_and_Styles {
	...
	public static function localize_script(){
		...
		$em_localized_js = array(
			...
			'ui_css' => plugins_url('includes/css/jquery-ui.min.css?vs=1', __FILE__),
		...
		if( get_option('dbem_rsvp_enabled') ){
		    ...
		    $em_localized_js = array_merge($em_localized_js, array(
				...
				//booking button
				'bb_full_da' =>  get_option('dbem_booking_button_msg_full_da'),
				...
				'bb_cancel_error_en' => get_option('dbem_booking_button_msg_cancel_error_en')
			));		
		}
		...
		if( is_user_logged_in() || is_page(get_option('dbem_edit_events_page')) ){
			...
			if( get_option('dbem_rsvp_enabled') ){
				...
				$em_localized_js['booking_warning_cancel'] = get_option('dbem_booking_warning_cancel_' . pll_current_language());
			}
		}
