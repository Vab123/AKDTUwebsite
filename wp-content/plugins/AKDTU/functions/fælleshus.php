 ...
 * 
 * @param DateTime|null $starttime_before Timestamp of when the events has to start before, treated as time in timezone 'Europe/Copenhagen', or null if no such time is required. Default: null
 * @param DateTime|null $starttime_after Timestamp of when the events has to start after, treated as time in timezone 'Europe/Copenhagen', or null if no such time is required. Default: null
 ...
 */
function get_common_house_events($starttime_before = null, $starttime_after = null, $endtime_before = null, $endtime_after = null, $limit = null, $status = 1, $orderby = null, $columns = ['event_id'])
{
	global $wpdb;
...
function get_recent_common_house_event_ids($limit = null, $status = 1)
{
	...
	return get_common_house_events($now, null, null, null, $limit, $status, 'event_end DESC', ['event_id']);
}
...
function get_next_common_house_event_ids($limit = null, $status = 1)
{
	...
	return get_common_house_events(null, $now, null, null, $limit, $status, 'event_end ASC', ['event_id']);
}
...
function get_current_common_house_event_ids($limit = null, $status = 1)
{
	...
	return get_common_house_events($now, null, null, $now, $limit, $status, null, ['event_id']);
}
 ...
 * 
 * @return string[] Array containing the following keys:
 ...
 * Performs the appropriate replaces in the name, defined by us.
 * 
 * @param EM_Event $event The event to output the name of.
 * @param string $calendar_language Locale of the language to write the name of the event in. (Default: "da_DK").
 * @param bool $is_export True if the name of the event is to be used when exporting the calendar to ics-format.
 * 
 * @return string Formatted version of the name of the event.
 */
function format_common_house_rental_description($event, $calendar_language = "da_DK", $is_export = false)
{
	$event_name_replaces = [
		'#APT' => padded_apartment_number_from_id($event->owner),
		'#_RENTAL_BEFORE_APARTMENTNUM' => pll_translate_string(($is_export ? 'EXPORT_' : '') . ($event->event_status == 0 ? 'RENTAL_BEFORE_APARTMENTNUM_NOTAPPROVED' : 'RENTAL_BEFORE_APARTMENTNUM_APPROVED'), $calendar_language),
		'#_RENTAL_AFTER_APARTMENTNUM' => pll_translate_string(($is_export ? 'EXPORT_' : '') . ($event->event_status == 0 ? 'RENTAL_AFTER_APARTMENTNUM_NOTAPPROVED' : 'RENTAL_AFTER_APARTMENTNUM_APPROVED'), $calendar_language),
		'&nbsp;' => ' ',
	];

	return str_replace(array_keys($event_name_replaces), $event_name_replaces, pll_translate_string(($is_export ? 'EXPORT_' : '') . ($event->event_status == 0 ? 'RENTAL_DESCRIPTION_NOTAPPROVED' : 'RENTAL_DESCRIPTION_APPROVED'), $calendar_language));
}
...
function get_price_to_pay($period_start, $period_end)
{
	...
	$price_to_pay = array_filter($price_to_pay, function ($val) {
		return $val > 0;
	});
...
function get_final_price($price_to_pay, $price_adjustments)
{
	...
	foreach ($price_to_pay as $username => $price) {
		 ...
		 */
		$final_price[$username] = $price + ($price_adjustments[apartment_number_from_username($username)] ?? 0);
	}
	...
	$posts_info = array_combine(
		...
		array_map(
			function ($params_per_language) {
				...
				# Protect event
				SwpmProtection::get_instance()->apply([$post_id], 'custom_post')->save();
				$level_ids = $wpdb->get_col("SELECT id FROM {$wpdb->prefix}swpm_membership_tbl WHERE id != 1");
				foreach ($level_ids as $level) {
					SwpmPermission::get_instance($level)->apply([$post_id], 'custom_post')->save();
				}
