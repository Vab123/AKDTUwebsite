...
function move_booking_to_user($user_id, $EM_Booking)
{
	...
	$wpdb->query("UPDATE {EM_BOOKINGS_TABLE} SET person_id = {$user_id} WHERE booking_id = {$EM_Booking->booking_id}");
}
...
function move_event_to_user($user_id, $EM_Event)
{
	...
	$wpdb->query("UPDATE {EM_EVENTS_TABLE} SET event_owner = {$user_id} WHERE event_id = {$EM_Event->event_id}");
