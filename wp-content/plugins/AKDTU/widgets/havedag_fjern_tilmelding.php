...
function fjern_tilmelding_til_havedag_dashboard_widget() {
	...
	if (!is_null($gardendays)) : ?>
		<table id="remove_from_gardenday_widget_table" width="100%" class="widefat">
			<colgroup>
			...
			<thead>
				<tr>
					<td style="vertical-align:middle;"><label>Begivenhed:</label></td>
					<td style="vertical-align:middle;"><select name="havedag_event" onchange="update_remove_gardenday_display(this.value)">
							<?php 
				...
				foreach ($gardendays as $gardenday) :
					foreach ($gardenday as $language => $event) : ?>
						<form action="" method="post" id="remove_gardenday_form_<?php echo $event->event_id; ?>">
							...
							<tr class="remove_gardenday_options alternate" id="remove_gardenday_<?php echo $event->event_id ?>_date" <?php if ($is_not_first) {
							...
							<tr class="remove_gardenday_options alternate" id="remove_gardenday_<?php echo $event->event_id ?>_signup" <?php if ($is_not_first) {
																																echo 'style="visibility:collapse"';
																															}; ?>>
								<td style="vertical-align:middle;"></td>
								<td style="vertical-align:middle;"><input type="submit" class="button-secondary" value="Afmeld"></td>
							</tr>
						</form>
