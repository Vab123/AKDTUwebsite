		...
		<table class="widefat">
			...
			<tbody>
				<tr class="alternate">
					...
					<td style="vertical-align:middle;"><input type="text" name="danish_name" value="Havedag - <?php echo (intval(date('n')) >= $start_showing_fall_from_month && intval(date('n')) <= $stop_showing_fall_after_month ? "Efterår " . date('Y') : "Forår " . (intval(date('n')) < $stop_showing_fall_after_month ? date('Y') : strval(intval(date('Y') + 1)))); ?>" style="width:100%;" /></td>
				</tr>
				<tr>
...
Hvis du har nogen spørgsmål eller kommentarer er du velkommen til at kontakte bestyrelsen på <a href="mailto:bestyrelsen@akdtu.dk">bestyrelsen@akdtu.dk</a>.</textarea></td>
				...
				<tr class="alternate">
					...
					<td style="vertical-align:middle;"><input type="text" name="english_name" value="Gardenday - <?php echo (intval(date('n')) >= $start_showing_fall_from_month && intval(date('n')) <= $stop_showing_fall_after_month ? "Fall " . date('Y') : "Spring " . (intval(date('n')) < $stop_showing_fall_after_month ? date('Y') : strval(intval(date('Y') + 1)))); ?>" style="width:100%;" /></td>
				</tr>
				<tr>
...
If you have any questions or comments, you are welcome to contact the Board on <a href="mailto:bestyrelsen@akdtu.dk">bestyrelsen@akdtu.dk</a>.</textarea></td>
				...
				<tr class="alternate">
					...
					<td style="vertical-align:middle;"><input type="datetime-local" name="publish_date" value="<?php echo (intval(date('n')) <= $stop_showing_fall_after_month ? date('Y') : strval(intval(date('Y') + 1))) . "-" . (intval(date('n')) >= $start_showing_fall_from_month && intval(date('n')) <= $stop_showing_fall_after_month ? str_pad(strval($stop_showing_fall_after_month), 2, "0", STR_PAD_LEFT) : str_pad(strval($start_showing_fall_from_month), 2, "0", STR_PAD_LEFT)) . "-" . "01" . "T" . "00" . ":" . "00"; ?>" style="width:100%;" /></td>
				</tr>
				<tr>
