...
if (isset($_REQUEST['action'])) {
	if ($_REQUEST['action'] == 'remove_board_deputy' && isset($_REQUEST['user'])){
		remove_board_deputy($_REQUEST['user']);
	}
}
 ...
 */
function remove_board_deputy($user_id){
	...
	if ($user_id > 0) {
		...
		$rows_changed = $wpdb->update($wpdb->prefix . 'AKDTU_board_deputies',array('end_datetime' => (new DateTime('now',new DateTimeZone('Europe/Copenhagen')))->format('Y-m-d H:i:s')), array('apartment_number' => apartment_number_from_id($_REQUEST['user']), 'end_datetime' => '9999-12-31 23:59:59'));

		# Write success message to admin interface
		if ($rows_changed > 0) {
			new AKDTU_notice('success','Bestyrelsessuppleanten blev fjernet');

			return true;
		}

		new AKDTU_notice('success','Bestyrelsessuppleanten blev ikke fjernet');
