...
if (isset($_REQUEST['action'])) {
	if ($_REQUEST['action'] == 'allow_renter_signup' && isset($_REQUEST['user']) && isset($_REQUEST['email']) && isset($_REQUEST['start_time']) && isset($_REQUEST['end_time'])) {
		allow_renter_signup($_REQUEST['user'], $_REQUEST['email'], $_REQUEST['start_time'], $_REQUEST['end_time']);
	}
}
 ...
 */
function allow_renter_signup($apartment_number, $email, $start_time, $end_time) {
	...
	if (renter_creation_permit_exists_where(["apartment_number" => $apartment_number])) {
		...
		if (create_renter_creation_permit($apartment_number, $email, $start_time, $end_time)) {
