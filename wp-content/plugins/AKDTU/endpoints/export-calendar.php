...
function AKDTU_export_calendar() {
	if (isset($_GET['user']) && isset($_GET['auth']) && username_exists($_GET['user']) && md5(get_user_by('login', ($_GET['user']))->user_pass) == $_GET['auth']) {
		...
		$events = get_common_house_events(null, null, null, null, null, [0, 1], "event_start_date ASC", ['event_id', 'post_id']);
		...
		header(($_SERVER['SERVER_PROTOCOL'] ?? 'HTTP/1.0') . ' 401 Unauthorized');
	}
