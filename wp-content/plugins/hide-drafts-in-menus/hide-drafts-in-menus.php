...
function r34hdm_get_unpublished_items($items) {
	...
	// Find posts in IDs list that are pending, draft or trash
	$sql = "SELECT `ID`, `post_title`, `post_status` FROM `" . $wpdb->prefix . "posts` WHERE `post_status` IN  ('pending', 'draft', 'trash', 'future') AND `ID` IN (" . implode(',',$ids) . ")";
				...
				foreach ((array)$menus as $menu_obj) {
					...
					if ($menu = wp_get_nav_menu_object($menu_obj->slug)) {
						...
						if (!empty($unpublished_items)) {
							foreach ((array)$unpublished_items as $unpublished) {
								$key = array_search($unpublished->ID, $ids);
								echo	'	#menu-item-' . $items[$key]->ID . ' .menu-item-handle { background: rgba(0,0,0,0.075); }' . "\n" .
										'	#menu-item-' . $items[$key]->ID . ' .item-type::before { content: \'[' . strtoupper($unpublished->post_status) . ']\00a0\00a0\'; font-weight: bold; }' . "\n";
							}
						}
					}
				}
