...
function check_auth() {
	...
	if ($protected->is_protected($post->ID)) {
		...
		if (!$auth->is_logged_in()) {
			...
			wp_redirect($login_page . '?visit=' . urlencode($_SERVER['REQUEST_URI']), 302);
			exit();
		}
	}
}
...
class SwpmAccessControl {
	...
	public function can_i_read_post($post) {
		...
		if (!$auth->is_logged_in()) {
			...
			$text = pll__('You need to login to view this content. ', 'simple-membership') . SwpmMiscUtils::get_login_link();
	...
	public function can_i_read_comment($comment) {
		...
		if (!$auth->is_logged_in()) {
			...
			$error_msg = '<div class="swpm-comment-not-logged-in">' . pll__('You need to login to view this content. ', 'simple-membership') . '</div>';
