...
class SimpleWpMembership {
	...
	public static function swpm_login($username, $pass, $rememberme = true) {
		...
		if (!SwpmUtils::is_ajax()) {
			if (!isset($_REQUEST['visit'])) {
				...
				$redirect_url = get_site_url() . urldecode($_REQUEST['visit']);
			}
	...
	public function notices() {
		...
		if ($message['succeeded']) {
			echo "<div class='statusnotice'><div class='em-warning em-warning-confirms notice notice-success'>";
			$succeeded = true;
		} else {
			echo "<div class='statusnotice'><div class='em-warning em-warning-errors notice notice-error'>";
		}
	...
	public function admin_library() {
		...
		$settings = array(
			'statusChangeEmailHead' => SwpmSettings::get_instance()->get_value('account-change-email-subject'),
