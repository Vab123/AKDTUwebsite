	...
	if (count($events) > 0): ?>
		<table id="tilføj_til_havedag_widget_table" width="100%">
			...
			<thead>
				...
				<tbody>
			<?php $is_not_first = false; foreach ($events as $event): ?>
				<form action="" method="post">
					...
					<tr class="tilføj_havedag_options" id="tilføj_havedag_<?php echo $event->event_id ?>_date" <?php if ($is_not_first){echo 'style="visibility:collapse"';}; ?>>
					...
					<tr class="tilføj_havedag_options" id="tilføj_havedag_<?php echo $event->event_id ?>_user" <?php if ($is_not_first){echo 'style="visibility:collapse"';}; ?>>
						<td><label>Lejlighed:</label></td>
						<td><?php apartments_dropdown(); ?></td>
					</tr>
					<tr class="tilføj_havedag_options" id="tilføj_havedag_<?php echo $event->event_id ?>_signup" <?php if ($is_not_first){echo 'style="visibility:collapse"';}; ?>>
						<td></td><td><input type="submit" class="button-secondary" value="Tilmeld"></td>
					</tr>
				</form>
