...
function show_board_widget() {
	...
	<table class="widefat">
		<colgroup>
			...
			<?php if (is_admin()):?><col span="1" style="width: 20%" /><?php endif; ?>
		</colgroup>
		<thead>
			<tr>
				<th>Lejlighed</th>
		...
		<tbody>
		<?php $row = 0; foreach ($board_members as $board_member): ?>
			<tr <?php if ($row % 2 == 0){echo 'class="alternate"';}; $row++; ?>>
				<td><?php echo ltrim(substr($board_member->user_name,4,3), "0"); ?></td>
				<td><?php echo $board_member->first_name . ' '  . $board_member->last_name; ?></td>
				<?php if (is_admin()):?><td><form action="" method="post" style="text-align:center">
