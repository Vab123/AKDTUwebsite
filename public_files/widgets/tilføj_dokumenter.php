		...
		<table id="tilføj_dokument_widget_table" width="100%">
			...
			<?php foreach ($document_types as $type): ?>
				<form action="" method="post" enctype="multipart/form-data">
					...
					<tr class="dokument_upload_options" id="document_<?php echo $type['short']; ?>_date" <?php if ($is_not_first){echo 'style="visibility:collapse"';}; ?>>
						<td><label>Dokument dato:</label></td>
						<td><input type="date" name="document_date" /></td>
					</tr>
					<?php if ($type['has_type']): ?>
						<tr class="dokument_upload_options" id="document_<?php echo $type['short']; ?>_typetype" <?php if ($is_not_first){echo 'style="visibility:collapse"';}; ?>>
					...
					<?php endif; ?>
					<tr class="dokument_upload_options" id="document_<?php echo $type['short']; ?>_file" <?php if ($is_not_first){echo 'style="visibility:collapse"';}; ?>>
						<td><label>Fil:</label></td>
						<td><input type="file" name="file" accept="application/pdf" /></td>
					</tr>
					<tr class="dokument_upload_options" id="document_<?php echo $type['short']; ?>_submit" <?php if ($is_not_first){echo 'style="visibility:collapse"';}; ?>>
						<td></td><td><input type="submit" class="button-secondary" value="Upload"></td>
					</tr>
				</form>
			<?php $is_not_first = true; endforeach; ?>
			</tbody>
		</table>
		<script>
			function update_upload_document_display(type) {
				let options = document.getElementsByClassName("dokument_upload_options");
				for (let option of options){option.style.visibility = "collapse";}
